...
def translate_text_chatgpt(text, src_lang, trg_lang):
    def build_fewshot(src_lang, trg_lang):
        ...
        fewshot_meggages = []
        for scr_text, trg_text in zip(src_examples, trg_examples):
            # for문을 2바퀴 돌게끔 만든다.
            fewshot_meggages.append({"role":"user", "content":src_lang})
            fewshot_meggages.append({"role":"assistant", "content":trg_lang})
        return fewshot_meggages

    system_instruction = f'assistant는 번역앱으로 독작한다. {src_lang}를 {trg_lang}로 적절하게 번역된 텍스트만 출력한다.'
...
with gr.Blocks() as demo:
    ...
    src_lang_dropdown = gr.Dropdown(choices = ['한국어', '영어', '일본어'], value = "영어", label = "번역할 내용을 선택하세요")
